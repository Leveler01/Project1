...
public class Stock extends JFrame implements ActionListener,KeyListener
{
...
public Stock()
   {
		...
		ll= new JLabel("Stock details");
		ll.setBounds(160,5,600,75);
...
if(b2==ae.getSource())
     {
         System.out.println("\n\tEvent occurs\n");
	 boolean r2=Stock.dosave(t1.getText(),t2.getText(),t3.getText(),t4.getText(),ta1.getText(),t5.getText());
         if(r2==true)
...
JOptionPane.showMessageDialog(null,"Record is not saved","Save Messege",JOptionPane.INFORMATION_MESSAGE);
 ...
 if(b3==ae.getSource())
     {
         System.out.println("\n\tEvent occurs\n");
	 Stock.doupdate(t1.getText(),t2.getText(),t3.getText(),t4.getText(),ta1.getText(),t5.getText());
      }

 if(b4==ae.getSource())
     {
         System.out.println("\n\tEvent occurs\n");
	 Stock.dodelete(t1.getText());
      }
...
public static boolean dosave(String s1,String s2,String s3,String s4,String s5,String s6)
{
boolean b=false;
try
        {
...
Class.forName("sun.jdbc.odbc.JdbcOdbcDriver"); 
          ...
          String sss="insert into Stock values("+s1+","+s2+",'"+s3+"',"+s4+","+s5+","+s6+")";     
...
try
	  ...
	  Statement st=con.createStatement();    
           String sss="update  Stock set    veh_id= "+s2+", Stock_date= '"+s3+"', min_qty = "+s4+", max_qty= "+s5+", Avail_qty = "+s6+"  where stock_id= "  + Integer.parseInt(s1);    
...
try
	  ...
	  Statement st=con.createStatement();
         String sss="delete from  Stock   where Stck_id= "  + Integer.parseInt(s1);    
  ...
  while(rs.next())
  {
	...
	if(rrr==true)
	{
                 JOptionPane.showMessageDialog(null,"Record is found","Search Query",JOptionPane.INFORMATION_MESSAGE);
                  String sss="Select * from Stock where Stock_id = "+y;
	 JOptionPane.showMessageDialog(null,sss,"Search Query",JOptionPane.INFORMATION_MESSAGE);
	ResultSet rs1=st1.executeQuery("Select * from Stock  where Stock_id = "+y);
	  while(rs1.next())
 	 {
  ...
  while(rs.next())
  {
    ...
    if(y==1)  {break;}
	 ...
	 JOptionPane.showMessageDialog(null,"First Record is found","Search Query",JOptionPane.INFORMATION_MESSAGE);
                	ResultSet rs1=st1.executeQuery("select * from Stock where Stock_id = "+id1);
	  while(rs1.next())
 	 {
                t1.setText(String.valueOf(rs1.getInt(1)));
        	t2.setText(String.valueOf(rs1.getInt(2)));
...
ResultSet rs=st.executeQuery("Select max(Stock_id) from Stock");
  ...
  { 
   ...
   y++; 
        ...
        JOptionPane.showMessageDialog(null,"New Record is added with Stock Id =" +y,"Search Query",JOptionPane.INFORMATION_MESSAGE);
...
public static  Stock s1;
public static void main(String s[])
	{
		 s1=new Stock();
s1.setTitle("Vehicle Showroom Sales System ........  Vehicle Showroom Sales System");
		s1.show();
	}//main ends
}//class ends