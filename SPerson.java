...
public class SPerson extends JFrame implements ActionListener,KeyListener,FocusListener
{
...
public SPerson()
   {
		...
		ll= new JLabel("Service Person Details");
...
hide();
 ...
 if(b2==ae.getSource())
     {
         System.out.println("\n\tEvent occurs\n");
	 SPerson.dosave(t1.getText(),t2.getText(),t3.getText(),t4.getText(),ta1.getText(),t5.getText(),t6.getText());
      }

 if(b3==ae.getSource())
     {
       
	 SPerson.doupdate(t1.getText(),t2.getText(),t3.getText(),t4.getText(),ta1.getText(),t5.getText(),t6.getText());
      }

 if(b4==ae.getSource())
     {
     
	 SPerson.dodelete(t1.getText());
      }
 if(b5==ae.getSource())
     {
...
ResultSet rs=st.executeQuery("Select max(spid) from Serviceperson");
  while(rs.next())
  { 
     y=rs.getInt(1);   
   } 
   y++; 
        ...
        JOptionPane.showMessageDialog(null,"New Record is added with spid =" +y,"Search Query",JOptionPane.INFORMATION_MESSAGE);
...
try
	  ...
	  Statement st=con.createStatement();
         String sss="insert into Serviceperson values("+s1+",'"+s2+"','"+s3+"','"+s4+"','"+s5+"','"+s6+"','"+s7+"')";     
...
try
	  ...
	  Statement st=con.createStatement();
         String sss="update  Serviceperson set  sp_name= '"+s2+"',  sp_addr= '"+s3+"', sp_mob= '"+s4+"', sp_phone = '"+s5+"', sp_mail= '"+s6+"',sp_gender = '"+s7+"'  where spid= "  + Integer.parseInt(s1);    
...
try
	  ...
	  Statement st=con.createStatement();
         String sss="delete from  Serviceperson   where spid= "  + Integer.parseInt(s1);    
...
ResultSet rs=st.executeQuery("Select * from Serviceperson");
  while(rs.next())
  {
	...
	if(rrr==true)
	{
                 JOptionPane.showMessageDialog(null,"Record is found","Search Query",JOptionPane.INFORMATION_MESSAGE);
                  String sss="select * from Serviceperson  where spid = "+y;
	 JOptionPane.showMessageDialog(null,sss,"Search Query",JOptionPane.INFORMATION_MESSAGE);
	ResultSet rs1=st1.executeQuery("select * from Serviceperson  where spid = "+y);
	  while(rs1.next())
 	 {
  ...
  while(rs.next())
  {
    ...
    if(y==1)  {break;}
	 ...
	 JOptionPane.showMessageDialog(null,"First Record is found","Search Query",JOptionPane.INFORMATION_MESSAGE);
                	ResultSet rs1=st1.executeQuery("select * from Serviceperson  where spid = "+id1);
	  while(rs1.next())
 	 {
   	  t1.setText(String.valueOf(rs1.getInt(1)));
        	 t2.setText(String.valueOf(rs1.getString(2)));
...
public static SPerson s1;
public static void main(String s[])
	{
		 s1 =new SPerson();
s1.setTitle("Vehicle Showroom Sales System ........  Service Person details");
		s1.show();
	}
}