...
public class Complaint extends JFrame implements ActionListener,FocusListener,KeyListener
{
   JLabel ll,l1,l2,l3,l4,l5,l6,l7,b,bic1,bic2;
...
public Complaint()
   {
		...
		ll= new JLabel("Complaints");
		ll.setBounds(150,3,600,75);
...
l7=new JLabel("Customer cont no");
...
if(b2==ae.getSource())
     {
         System.out.println("\n\tEvent occurs\n");
	 Complaint.dosave(t1.getText(),t2.getText(),t3.getText(),t4.getText(),ta1.getText());
      }

 if(b3==ae.getSource())
     {
         System.out.println("\n\tEvent occurs\n");
	 Complaint.doupdate(t1.getText(),t2.getText(),t3.getText(),t4.getText(),ta1.getText());
      }

if(b4==ae.getSource())
     {
         System.out.println("\n\tEvent occurs\n");
	 Complaint.dodelete(t1.getText());
      }
...
try
	  ...
	  Statement st=con.createStatement();
         String sss="insert into Complaint values("+s1+","+s2+","+s3+",'"+s4+"','"+s5+"')";     
...
try
	  ...
	  Statement st=con.createStatement();
         String sss="update  Complaint set  cust_id= "+s2+",  vid= "+s3+", compdate= '"+s4+"', details = '"+s5+"' where comp_id= "  + Integer.parseInt(s1);    
...
try
	  ...
	  Statement st=con.createStatement();
         String sss="delete from  Complaint   where comp_id = "  + Integer.parseInt(s1);    
  ...
  while(rs.next())
  {
	...
	if(rrr==true)
	{
                 JOptionPane.showMessageDialog(null,"Record is found","Search Query",JOptionPane.INFORMATION_MESSAGE);
                  String sss="select * from Complaint  where comp_id = "+y;
	 JOptionPane.showMessageDialog(null,sss,"Search Query",JOptionPane.INFORMATION_MESSAGE);
	ResultSet rs1=st1.executeQuery("select * from Complaint where comp_id = "+y);
	  while(rs1.next())
 	 {
  ...
  while(rs.next())
  {
    ...
    if(y==1)  {break;}
	 ...
	 JOptionPane.showMessageDialog(null,"First Record is found","Search Query",JOptionPane.INFORMATION_MESSAGE);
                	ResultSet rs1=st1.executeQuery("select * from Complaint  where comp_id= "+id1);
	  while(rs1.next())
 	 {
   	       t1.setText(String.valueOf(rs1.getInt(1)));
        	 t2.setText(String.valueOf(rs1.getInt(2)));
...
ResultSet rs=st.executeQuery("Select max(comp_id) from Complaint");
  while(rs.next())
  { 
     y=rs.getInt(1);   
   } 
   y++; 
        ...
        JOptionPane.showMessageDialog(null,"New Record is added with comp_id =" +y,"Search Query",JOptionPane.INFORMATION_MESSAGE);
...
ResultSet rs=st.executeQuery("Select * from  Customer where custid = "+cid);
	  ...
	  while(rs.next())
 	 {
   	    t5.setText(String.valueOf(rs.getString(2)));
        	 t6.setText(String.valueOf(rs.getInt(3)));
         	break;
...
public static  Complaint s1;
public static void main(String s[])
	{
		 s1 =new Complaint();
s1.setTitle("Vehicle Showroom Sales System ........ Complaint Details");
		s1.show();
	}
}